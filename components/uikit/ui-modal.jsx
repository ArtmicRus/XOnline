 ...
 * }} props
 */
export function UiModal({ width, className, children, isOpen, onClose }) {
  // e - React ивент который мы отслеживаем от браузера
  // Во всех браузерах свои ивенты, React преобразует их к своему стандартному виду
  const handleClick = (e) => {
    // Внутри target лежит информация о блоке на который было произведено нажатие
    // Чтобы не закрывать модальное окно когда на него мы проверяем если есть наш data-id то мы не закрываем модалку
    // на при клике на другой объект закрываем
  ...
  const modal = (
    ...
    >
      <div
        data-id="modal"
        className={clsx(
          "bg-white rounded-lg min-h-[320px] relative",
          "flex flex-col",
          {
...
/**
 * Так как этот компонент НЕ МОЖЕТ использоваться без модального окна мы его прицепляем к нему
 * @param children
 * @param className
 * @returns {JSX.Element}
 * @constructor
