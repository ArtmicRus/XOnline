...
function PlayerInfo({ playerInfo, isRight, isTimerRunning }) {
  ...
  useEffect(() => {
    if (isTimerRunning) {
      ...
      // Функция очистки - получается сбрасывает уменьшение таймера при изменении состояния isTimerRunning
      // Эта функция вызывается ПЕРЕД СЛЕДУЮЩИМ срабатыванием эффекта - то есть она аннулирует уменьшение текущего таймера при запуске следующего
      return () => {
  ...
  const getTimerColor = () => {
    if (isTimerRunning) {
      return isDander ? "text-orange-600" : "text-slate-900";
    }
